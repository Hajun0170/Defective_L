...
public class RocketBoss : MonoBehaviour
{
    ...
    private SpriteRenderer spriteRenderer; // 보스 몸체 뒤집기용 

    // 원래 크기를 기억할 변수
    private Vector3 originalScale;

    [Header("Aim Offset")]
    public float aimOffset = 0.75f; // 0.75만큼 위를 조준함 (캐릭터의 가슴 높이)
...
private List<GameObject> activeProjectiles = new List<GameObject>();
    ...
    private void Update()
    {
        ...
        // 공격 로직
        // 사거리 안이고, 공격 중이 아니고, 로켓이 장전되어 있을 때
   ...
   // 팔 회전 (조준) 
    ...
    IEnumerator AttackRoutine()
    {
        ...
        // 뜸 들이기 (Update에서 계속 조준)
        yield return new WaitForSeconds(castTime);

        // 대기 시간이 끝난 직후 보스가 이미 죽었거나 비활성화되었다면 중단
...
// OnDisable에서도 처리 (씬 전환이나 비활성화 시 유령 판정 방지)
